...
void setup_pipes_and_redirections(int i, int num_commands, int *pipefd, int *prev_pipe_read, CommandData *data)
{
    int fd;
    ...
    {
        fd = open(data->output_file, O_WRONLY | O_CREAT | (data->append_output ? O_APPEND : O_TRUNC), 0644);
        if (fd < 0)
        {
    ...
    while (i < data->num_commands)
    {
        ...
        if (data->num_commands > 1 || !is_builtin_cmd)
        {
            ...
            if (pid == 0)
            {
                ...
                if (is_builtin_cmd)
                {
                    execute_builtin(data->commands[i], data->arguments[i], &i, envp, vars, data);
                    exit(g_exit_status);
                }
                else
                {
